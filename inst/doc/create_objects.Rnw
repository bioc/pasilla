...
\\
You can find the raw fastq files in \url{www.embl.de/~reyes/Graveley/fastq}.
...
The FASTQ files were aligned using TopHat version 1.2.0 with default parameters against the 
reference Drosophila melanogaster genome.
summarizes the read number and alignment statistics.
Table~\ref{tab:samples} 
\input{create_objects_tabsamples.tex}
\subsection{How to generate an alignment with tophat}
We can build the indexed form of a reference fasta fileto make a bowtie index that is
required to align the reads with tophat. The reference genomic DNA fasta files can be 
obtained directly from the ensembl ftp server.  
\footnote{\url{http://www.ensembl.org/info/data/ftp/index.html}}. Having this we can run bowtie-build to index the
fasta file, and we now ready to make the alignment. For more detailed information of how to generate this index
please visit bowtie webpage \footnote{\url{http://bowtie-bio.sourceforge.net/tutorial.shtml}}.
\scriptsize
\begin{lstlisting}
wget \
ftp://ftp.ensembl.org/pub/release-62/fasta/drosophila_melanogaster/ \
...
bowtie-build Drosophila_melanogaster.BDGP5.25.62.dna_rm.toplevel.fa d_melanogaster_BDGP5.25.62
\end{lstlisting}
\normalsize
An example of how to generate the alignment bam file using tophat with default parameters for single-reads:
\scriptsize
\begin{lstlisting}
tophat /path/to/bowtieindex/bowtie_index \
reads1.fastq,reads2.fastq,...,readsN.fastq
\end{lstlisting}
\normalsize
And for paired ends samples:
\scriptsize
\begin{lstlisting}
tophat -r inner-fragment-size
/path/to/bowtieindex/bowtie_index \
reads1_1.fastq,reads2_1.fastq,...,readsN_1.fastq \
reads1_2.fastq,reads2_2.fastq,...,readsN_2.fastq
\end{lstlisting}
\normalsize
for more detail information of tophat visit the software webpage. \footnote{\url{http://tophat.cbcb.umd.edu/tutorial.html}}
The SAM alignments from which \Rpackage{pasilla} was generated are
available in \url{www.embl.de/~reyes/Graveley/bam}. 

\section{Exon count files}
To generate per-exon read counts, we first needed to define the exonic regions.  
To this end, we downloaded the file Drosophila\_melanogaster.BDGP5.25.62.gtf.gz from 
...
contained in pasilla/extdata. For example, for this file we would do:
\scriptsize
\begin{lstlisting}
wget \
ftp://ftp.ensembl.org/pub/release-62/gtf/ \
...
(sorted by position in the case of a paired end data) and the \texttt{gtf} file 
Dmel.BDGP5.25.62.DEXSeq.chr.gff and returned one file for each biological replicate 
with the exon counts. For example, for the file treated1.bam, which is single-end
alignment, we would do:
...
python dexseq_count.py Dmel.BDGP5.25.62.DEXSeq.chr.gff treated1.sam treated1fb.txt

\end{lstlisting}
\normalsize
For the file treated2.bam, which is a paired-end alignment:
...
sort -k 1,1 -k2,2n treated2.sam > treated2_sorted.bam
python dexseq_count.py -p yes Dmel.BDGP5.25.62.DEXSeq.chr.gff treated2_sorted.sam treated2fb.txt

\end{lstlisting}

\normalsize
The output of the two HTSeq python scripts is already provided in the 
\Rpackage{pasilla} package:
%
...
\footnote{\url{http://www-huber.embl.de/users/anders/HTSeq/doc/overview.html}}
